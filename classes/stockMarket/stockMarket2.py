...
def show_stock(list_of_stock_items):
    ...
    for item in list_of_stock_items:
        ...
        if item.get_name() == 'Rabbit':
            rabbit_amounts.append([item.get_amount()])
    ...
    stack_sums = [0] * len(rabbit_amounts)

    for rabbit_amount in rabbit_amounts:
        ax.bar(['hamster'], rabbit_amount, bottom=stack_sums)
        stack_sums = get_bottom_list(stack_sums, rabbit_amount)
